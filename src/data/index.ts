/**
 * Renovativ AI Map — typed loader for the deterministic US sample dataset.
 *
 * The dataset is generated by scripts/generate-buildings-us.mjs (seeded,
 * reproducible) and conforms to the Building interface in src/types/index.ts.
 * The certificate `ep` field carries a HERS-style index score.
 *
 * No runtime dependencies: pure TypeScript over the bundled JSON.
 */

import type { Building, EnergyLabel } from '../types';
import buildingsJson from './buildings-us.json';

const BUILDINGS: Building[] = buildingsJson as unknown as Building[];

const SEARCH_LIMIT = 20;

/** Lowercase + strip accents so "Main", "main" and "Peachtree"/"peachtree" all match. */
...
/** Counts per HERS band (A to G) and per city, for the map legend and dashboards. */
