...
/** Heatwave tab: summer comfort at three horizons and preparation measures. */
export default function HeatwaveTab({ building }: HeatwaveTabProps) {
  const horizons = comfortForHorizons(building);

  const recs = useMemo(
    () =>
      HEATWAVE_RECS_UK.filter((r) => matchesTrigger(building, r.trigger)).sort(
        (a, b) => PRIORITY_ORDER[a.priority] - PRIORITY_ORDER[b.priority],
      ),
    [building],
  );

  const items = REGULATION_UK.filter((r) => r.relevance.includes('heatwave'));
  ...
  return (
    <>
      <section className="detail-panel__section">
        ...
        <p className="note">
          Overheating degree-hours per summer, without air conditioning. The
          2050 and 2100 projections include climate warming and the urban heat
          island effect.
        </p>
