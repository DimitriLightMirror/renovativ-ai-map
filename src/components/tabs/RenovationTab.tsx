...
function Chapelet({ results }: { results: ScenarioResult[] }) {
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Renovation gesture ranking"
    >
      {results.map((r, i) => {
        const y = i * rowHeight;
...
/** Renovation tab: objective, ranked chart, recommended scenario, incentives. */
export default function RenovationTab({ building }: RenovationTabProps) {
  ...
  const items = REGULATION_US.filter(
  ...
  return (
    <>
      ...
      <section className="detail-panel__section">
        ...
        {top.length > 0 ? (
          <>
            ...
            <p className="note">
              The 10 most relevant gestures for this objective, scored out of
              100. Hover a bar for gesture details and payback time.
            </p>
          </>
        ) : (
      ...
      <section className="detail-panel__section">
        <h3>Standards and incentives</h3>
        {items.map((item) => (
